...
## 修订二(2026-07-16,用户实测话题标签药丸)

实测 https://mp.weixin.qq.com/s/1B516kGEeEuKMJSxmJ6G-A 的话题标签:蓝底白字药丸
(background rgb(26,115,232) + color #fff + 圆角内边距),相邻药丸之间无任何空格。
浅色门槛滤掉深蓝底、灰阶规则滤掉白字 → 双双降级成粘连裸文字。

- **成对配色规则**:元素同时带彩色背景 + **刻意的文字色**(彩色,或浅色如白字;
  编辑器默认近黑字不算,不复活噪音),且明暗对比足够(感知亮度差 ≥ 90)
  → 成对保留 `<mark style="background-color:X;color:Y">`,背景不再受浅色门槛
  限制——文字色是作者自带的,不依赖强制深字,亮暗主题自洽。
  注:亮度差 ≥ 90 是防病态门槛(同色对/近同色对),不是 WCAG 对比度保证;
  成对保留复现的是作者原始对比,忠实优先
- 单独背景(无文字色)→ 维持浅色门槛;单独白/灰字 → 仍过滤
- 对比不足(亮度差 < 90)→ 回退到单项规则各自判定
- **嵌套去重重申字色**:内层底色不同、字色与外层相同时,字色必须在内层 mark
  上重申——绝不产出「只有深底没有字色」的不可读形态(评审确认项)
- 拼接时相邻两个 mark 之间补一个空格(**经验上**真实文章不存在「同一高亮被
  拆成相邻两段」——两篇实测样本为证,非结构上不可能;药丸不再粘连)
- 白名单/CSS 零改动:双色 mark 形态已在白名单;内联色优先级高于样式表的强制深字
- 补充 YAGNI 映射:背景在祖先、文字色在后代的「跨层药丸」不做跨层合并
  (同基础设计的跨层 YAGNI),该形态仍降级为裸文字
- 交叉引用:「评审后修正」中"深红底白字不转 mark"的示例已被本节成对规则取代

## 修订三(2026-07-18,用户实测标题颜色)

实测 https://mp.weixin.qq.com/s/3Xz7Gzovx6ZkGXI2WZONTg:h2/h3 自带
`color: #773098` 紫色,但标题分支走 get_text 纯文字通道,颜色被丢。

- **标题复用刻意强调成色规则**:保留色取自 h1-h6 元素自身 style,
  或**包住整个标题文字的内层元素**(评审实证:示例文章 26 个彩色标题中
  18 个色在标题自身、8 个藏在内层 strong,仅 3 个真素面)→
  `## <span style="color:#xxxxxx">标题</span>`(mark/双色同理复用 _wrap_color);
  黑白灰默认色照旧过滤,输出保持素面;标题内部换行折叠为空格(防块切分
  撕开色标签)
- 浮动目录(TOC)条目一律纯文字:标题里的色标签不带进目录面板
- 明确不搬(页面级主题,搬了会顶掉阅读排版与暗色模式):网格纸背景、
  专用字体、标题居中;伪标题(样式冒充的 section)暂不成色,真实病例再说
